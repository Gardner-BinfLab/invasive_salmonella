...
row.names(eggNOG) <- c("Typhimurium", "Newport", "Heidelberg", "Schwarzengrund", "Agona", "Enteritidis", "ParatyphiB", "TyphiCT18", "ParatyphiA", "Dublin", "Gallinarum", "Choleraesuis", "ParatyphiC")

# compute delta-bitscore for assessment of functional impacts of sequence variation
eggNOG2 <- data.frame()
...
colnames(eggNOG2) <- c("Typhimurium", "Newport", "Heidelberg", "Schwarzengrund", "Agona", "Enteritidis", "ParatyphiB", "TyphiCT18", "ParatyphiA", "Dublin", "Gallinarum", "Choleraesuis", "ParatyphiC")

traineggNOG <- t(eggNOG2)
...
traineggNOG2 <- data.frame(class=traineggNOG$class)
for(i in 1:(ncol(traineggNOG)-1)) {
  if(sum(is.na(traineggNOG[,i]))<7) {
    traineggNOG2 <- cbind.data.frame(traineggNOG2, traineggNOG[,i])
    colnames(traineggNOG2)[ncol(traineggNOG2)] <- colnames(traineggNOG)[i]
  }
}
row.names(traineggNOG2) <- row.names(traineggNOG)

```

```{r}
######## ALTERNATIVE APPROACH WITH MISSING GENES SCORED AS ZERO #############
# use of this approach or the one above depends on the quality of your genomes - if no reads for a gene is likely to indicate that the gene has been deleted, use this approach, but if it may indicate poor coverage, use the above approach
...
save(model, train2, train3, train4, file="traindata.Rdata")
